...
export class InterviewController {
  ...
  @Post('start')
  @ApiOperation({ summary: 'Start a new voice profile interview session' })
  start(@CurrentUser() user: any, @Body() dto: StartInterviewDto) {
    return this.interviewService.startInterview(user.id, dto);
  }

  @Get('next-question')
  @ApiOperation({ summary: 'Get the next question for an active session' })
  ...
  @Post('complete')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Mark interview as complete and trigger voice synthesis' })
